...
def get_students_with_discipline(discipline_id):
    students = (
        session.query(Student, Grade.Grade_Value)
    ...
    if students:
        print(f"🎓 Студенти з оцінками для дисципліни ID {discipline_id}:")
        for student, grade in students:
            print(f"   {student.First_Name} {student.Last_Name}: {grade}")
    else:
        print(f"❌ Немає студентів з оцінками для дисципліни ID {discipline_id}!")
...
if __name__ == "__main__":
    ...
    elif command == "get_students_with_discipline":
        discipline_id = int(sys.argv[2])
        get_students_with_discipline(discipline_id)
